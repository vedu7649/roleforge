...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (state?.interviewResult) {
      const { result, weakAreas } = state.interviewResult;
      if (result === 'fail') {
        setSkillIntelligence({
          strength: "Foundational structure looks good",
          weakness: `Challenged by: ${weakAreas.join(', ')}`,
          adjustment: "Injecting specialized revision tasks into Dashboard..."
        });
      } else {
        setSkillIntelligence({
          strength: "Mastered latest checkpoint!",
          weakness: "No major gaps detected",
          adjustment: "Full speed ahead to next phase"
        });
      }
    }
